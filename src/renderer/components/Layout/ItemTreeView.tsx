...
export default function ItemTreeView() {
  ...
  const activeNode = useSelector(
    (state: { item: { activeNode: TreeNode | null } }) => state.item.activeNode
  );
  const expandedItemIds = useSelector(
    (state: { item: { expandedItemIds: string[] } }) => state.item.expandedItemIds
  );
